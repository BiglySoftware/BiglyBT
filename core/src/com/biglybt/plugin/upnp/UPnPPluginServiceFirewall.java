...
public class 
UPnPPluginServiceFirewall
	implements UPnPPluginService
{
	private final UPnPWANIPv6FirewallControl		firewall;
	...
	protected AEMonitor	this_mon 	= new AEMonitor( "UPnPPluginServiceFirewall" );

	protected
	UPnPPluginServiceFirewall(
		UPnPWANIPv6FirewallControl		_firewall,
	...
	getGenericService()
	{
		return( firewall.getGenericService());
	}
	
	public String
	getName()
	{
		return( getGenericService().getDevice().getRootDevice().getDevice().getFriendlyName());
	}

	public String
	getInfo()
	{
		return( getGenericService().getDevice().getRootDevice().getInfo());
	}
	public String
	getAddress()
	{
		return( getGenericService().getDevice().getRootDevice().getLocation().getHost());
	}

	public int
	getPort()
	{
		URL	url = getGenericService().getDevice().getRootDevice().getLocation();
	...
	checkMapping(
		...
		UPnPMapping			mapping )
	{
		InetAddress local_ia = firewall.getGenericService().getDevice().getRootDevice().getLocalAddress( false );
		
		if ( local_ia == null ){
			
				// no local IPv6 address
	...
	removeMapping(
		...
		ServiceMappingImpl		service_mapping,
		boolean				end_of_day )
	{
		if ( service_mapping.isExternal()){

			log.log( "Pinhole " + service_mapping.getString() + " not removed as not created by " + Constants.BIGLYBT_NAME );
			...
			if ( end_of_day && persistent == UPnPMapping.PT_PERSISTENT ){
				...
				log.log( "Pinhole " + service_mapping.getString() + " not removed as mapping is persistent" );
				...
				if ( service_mapping.getMappings().size() == 0 ){
					...
					try{
						firewall.removePinhole(
								service_mapping.isTCP(), service_mapping.getPort());

						log.log( "Pinhole " + service_name + " removed" );

					}catch( Throwable e ){

						log.log( "Pinhole " + service_name + " failed to delete", e );
					}
					...
					if ( !end_of_day ){
						...
						log.log( "Pinhole " + service_mapping.getString() + " not removed as interest remains" );
					}
				}
			}
		}
	...
	getMappings()
	{
		try{
			...
			return( service_mappings.toArray( new ServiceMappingImpl[service_mappings.size()]));
	...
	getString(
		UPnPMapping		mapping )
	{
		return( "IPv6: " + mapping.getString());
	}
		
	public String
	getString()
	{
		String str = "name=" + getName() + ",info=" + getInfo() + ",int=" + getAddress() + ":" + getPort() + ",ext=" + getExternalIPAddress();

		ServiceMappingImpl[] sms = getMappings();

		for ( ServiceMappingImpl sm: sms ){
		...
		implements ServiceMapping
	{
		...
		protected
		ServiceMappingImpl(
			UPnPWANIPv6FirewallControlPinhole		device_pinhole )
		{
			tcp				= device_pinhole.isTCP();
			port			= device_pinhole.getExternalPort();
			internal_host	= device_pinhole.getInternalHost();

			String	desc = device_pinhole.getDescription();
