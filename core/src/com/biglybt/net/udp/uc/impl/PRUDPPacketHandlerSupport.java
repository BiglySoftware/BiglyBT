...
import com.biglybt.net.udp.uc.impl.PRUDPPacketHandlerImpl.PacketTransformer;
	...
	implements PRUDPPacketHandler
{
	...
	private final CopyOnWriteList<PRUDPPrimordialHandler>	primordial_handlers;
	
	private final PRUDPPacketHandlerStatsImpl	stats;
	...
	{
		this( port, bind_ip, packet_transformer, null, null, null );
	}
	...
	PRUDPPacketHandlerSupport(
		...
		PRUDPPacketHandlerStatsImpl				_stats,
		CopyOnWriteList<PRUDPPrimordialHandler>	_primordial_handlers,
		PRUDPRequestHandler						_request_handler )
	{
		...
		if ( _stats != null ){
			...
			stats = new PRUDPPacketHandlerStatsImpl( this );
		}
		
		if ( _primordial_handlers != null ){
			
			primordial_handlers = _primordial_handlers;
			
		}else{
			
			primordial_handlers = new CopyOnWriteList<>();
		}
		...
		AESemaphore init_sem = new AESemaphore("PRUDPPacketHandler:init");
				...
				run()
				{
					...
					while( true ){
						...
						if ( destroyed ){
							...
							// something went wrong (e.g. bind fail). We don't want to just die though as nothing
							// will restart things should the failure be transient
						...
						synchronized( PRUDPPacketHandlerSupport.this ){
							...
							is = new AESemaphore( "PRUDPPacketHandler:init" );
						
							destroy_sem	= new AESemaphore( "PRUDPPacketHandler:destroy" );
							
							failed		= false;
							init_error	= null;
						}
						
						Logger.log(new LogEvent(LOGID, "PRUDPPacketReceiver: Re-attempting setup for " + port + ", bind=" + target_bind_ip ));
					}
				}
			}.start();
	...
	calcBind()
	{
		if ( explicit_bind_ip != null ){
			...
			if ( explicit_bind_ip_ad ){
				...
				if(altProtocolDelegate != null && !altProtocolDelegate.getExplicitBindAddress().equals(altAddress)){
				...
				if(altAddress != null && altProtocolDelegate == null){
					altProtocolDelegate = PRUDPPacketHandlerImpl.createPacketHandler(port,altAddress,packet_transformer,stats,primordial_handlers,request_handler );
				}
		...
		throws SocketException
	{
		...
		socket.bind( new InetSocketAddress(new InetSocketAddress(0).getAddress(), port ));
			...
			while( !( failed || destroyed )){
				...
				try{
					if ( target_bind_ip == null ){
						...
						new_socket = getSocket( port );

					}else{

						address = new InetSocketAddress( target_bind_ip, port );

						new_socket = getSocket( address );
					}
						...
						if ( target_bind_ip.isAnyLocalAddress()){
							...
							if ( guess != null ){
								...
								try{
									...
									new_socket = getSocket( guess_address );
		...
		throws PRUDPPacketHandlerException
	{
		...
		if ( delegate != null && destination_address.getAddress().getClass().isInstance(delegate.getExplicitBindAddress())){
		...
		throws PRUDPPacketHandlerException
	{
		...
		if ( 	delegate != null &&
				target.getAddress().getClass().isInstance(delegate.getExplicitBindAddress())){
