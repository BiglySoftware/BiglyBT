...
public class LongSwtParameter
	extends BaseSwtParameter<LongSwtParameter, Long>
{
	public interface ValueProcessor
		extends SwtParameterValueProcessor<LongSwtParameter, Long>
	{
		public default Long
		getValue(
			List<Long>	values )
		{
			if ( values.isEmpty()){
				...
				for ( long v: values.subList( 1, values.size())){
					if ( v != result ){
						return( null );
					}
				}
	...
	public LongSwtParameter(Composite composite, LongParameterImpl param) {
		this(composite, param.getConfigKeyName(), param.getLabelKey(),
				param.getSuffixLabelKey(),
				param.isLimited() ? param.getMinValue() : Long.MIN_VALUE,
				param.isLimited() ? param.getMaxValue() : Long.MAX_VALUE,
				param.isStoredAsString() ? new ValueProcessor() {
					@Override
					public Long getValue(LongSwtParameter p) {
						return null;
					}

					@Override
					public boolean setValue(LongSwtParameter p, Long value) {
						return false;
					}
				} : null);
	...
	public LongSwtParameter(Composite composite, String paramID, String labelKey,
			String suffixKey,
			SwtParameterValueProcessor<LongSwtParameter, Long> valueProcessor) {
		this(composite, paramID, labelKey, suffixKey, Long.MIN_VALUE,
				Long.MAX_VALUE, valueProcessor);
	}
	 ...
	 */
	public LongSwtParameter(Composite composite, String paramID, String labelKey,
			String suffixLabelKey, long minValue, long maxValue,
			SwtParameterValueProcessor<LongSwtParameter, Long> valueProcessor) {
		super(paramID);
		...
		timerEventSave = new TimerEventPerformer() {
			...
			public void perform(TimerEvent event) {
				Utils.execSWTThread(() -> {
					...
					setValue((long)spinner.getSelection());
				});
			}
		};
		...
		// obviously Spinner doesn't support Longs so need to switch this to text box oneday :)
		...
		spinner.addListener(SWT.Modify, event -> {
			...
			try {
				long val = text.isEmpty() ? valueWhenBlank : Long.parseLong(text);
				validate(val);
			} catch (Throwable t) {
	...
	public void setMinimumValue(int value) {
		...
		Utils.execSWTThread(() -> {
			...
			if (iMinValue != Integer.MIN_VALUE && getLongValue() < iMinValue) {
				setValue(iMinValue);
			}
		});
	}
	...
	public void setMaximumValue(int value) {
		...
		Utils.execSWTThread(() -> {
			...
			if (iMaxValue != Integer.MAX_VALUE && getLongValue() > iMaxValue) {
				setValue(iMaxValue);
			}
		});
	}
	...
	@Override
	public ValidationInfo validate(Long newValue) {
	...
	public void refreshControl() {
		...
		Utils.execSWTThread(() -> {
			...
			if (spinner.getSelection() != value.intValue()) {
				if (DEBUG) {
